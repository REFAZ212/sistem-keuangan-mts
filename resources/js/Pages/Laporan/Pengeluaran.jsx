...
export default function Pengeluaran({ laporan, accounts, filters }) {
    ...
    const submit = (e) => { e.preventDefault(); get(route('laporan.pengeluaran'), { preserveState: true, preserveScroll: true }); };
            ...
            header={
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-0.5 text-[13px] text-gray-500">Daftar pengeluaran yang sudah disetujui</p>
                    </div>
                    <div className="flex gap-2">
                        <a href={route('export.pengeluaran.pdf', filters)} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-[13px] font-semibold text-gray-700 transition hover:bg-gray-50">
                            <Icon name="download" className="h-4 w-4" /> PDF
                        </a>
                        <a href={route('export.pengeluaran.excel', filters)} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-[13px] font-semibold text-gray-700 transition hover:bg-gray-50">
        ...
        >
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:p-5">
                <form onSubmit={submit} className="flex flex-wrap items-end gap-3">
                    ...
                    <button type="button" onClick={() => window.location.href = route('laporan.pengeluaran')} className="inline-flex items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-4 py-2 text-[13px] font-medium text-gray-600 transition hover:bg-gray-50"><Icon name="x" className="h-3.5 w-3.5" /> Reset</button>
                </form>
            </div>
            ...
            <div className="mt-4 rounded-2xl border border-gray-100 bg-rose-50 p-5 shadow-sm">
                <p className="text-[11px] font-bold uppercase tracking-wider text-rose-500">Total Pengeluaran</p>
                <p className="mt-1 text-3xl font-bold text-rose-700">Rp {fmt(laporan.total)}</p>
            </div>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {laporan.pengeluaran.map((item) => (
                                    <tr key={item.id} className="transition hover:bg-gray-50/50">
                                        ...
                                        <td className="px-5 py-3.5 text-right text-[13px] font-bold text-rose-600">Rp {fmt(item.jumlah)}</td>
                                        ...
                                        <td className="px-5 py-3.5 text-center">
                                            <a href={route('export.kuitansi.pengeluaran', item.id)} className="inline-flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-2.5 py-1 text-[11px] font-medium text-gray-600 transition hover:bg-gray-50">
