...
export default function Create({ accounts, kategori, nomorTransaksi }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('penerimaan.store'), { onSuccess: () => reset() });
    };
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-xl font-bold text-gray-900">Tambah Penerimaan</h1>
                        <p className="mt-0.5 text-[13px] text-gray-500">Buat transaksi pemasukan baru</p>
                    </div>
                    ...
                    <form onSubmit={submit} className="space-y-5">
                        ...
                        <label className="flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50 px-4 py-3.5">
                            ...
                            <span className="text-[13px] leading-snug text-gray-600">
                                Setujui sekaligus <span className="font-semibold text-gray-800">(tidak bisa diubah setelah disetujui)</span>
                            </span>
                        </label>
                        <div className="flex justify-end gap-3 border-t border-gray-100 pt-5">
                            <Link href={route('penerimaan.index')} className="rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-[13px] font-semibold text-gray-700 transition hover:bg-gray-50">
                                Batal
                            </Link>
                            <button type="submit" disabled={processing} className="rounded-xl bg-emerald-600 px-5 py-2.5 text-[13px] font-semibold text-white shadow-lg shadow-emerald-600/20 transition hover:bg-emerald-700 disabled:opacity-50">
                                Simpan
                            </button>
                        </div>
                    </form>
