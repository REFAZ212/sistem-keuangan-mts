...
export default function Edit({ pengeluaran, accounts, kategori }) {
    const { data, setData, put, processing, errors, reset } = useForm({
        nomor_transaksi: pengeluaran.nomor_transaksi, tanggal: pengeluaran.tanggal,
        account_id: pengeluaran.account_id, kategori_id: pengeluaran.kategori_id || '', jumlah: pengeluaran.jumlah,
        keterangan: pengeluaran.keterangan || '', status: pengeluaran.status,
    });
    const submit = (e) => { e.preventDefault(); put(route('pengeluaran.update', pengeluaran.id), { onSuccess: () => reset() }); };
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-xl font-bold text-gray-900">Edit Pengeluaran</h1>
                        <p className="mt-0.5 text-[13px] text-gray-500">Ubah data transaksi pengeluaran</p>
                    </div>
            ...
            <div className="mx-auto max-w-2xl">
                <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm sm:p-8">
                    <form onSubmit={submit} className="space-y-5">
                        <div><InputLabel value="Nomor Transaksi" /><TextInput value={data.nomor_transaksi} readOnly className="mt-1.5 block w-full bg-gray-50" /></div>
                        ...
                        <div><InputLabel value="Jumlah" /><div className="relative mt-1.5"><span className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5 text-[13px] text-gray-400 font-medium">Rp</span><TextInput type="number" step="0.01" min="0" value={data.jumlah} onChange={(e) => setData('jumlah', e.target.value)} className="block w-full !pl-10" /></div><InputError message={errors.jumlah} className="mt-1" /></div>
                        ...
                        <label className="flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50 px-4 py-3.5">
                            ...
                            <span className="text-[13px] leading-snug text-gray-600">Setujui <span className="font-semibold text-gray-800">(tidak bisa diubah setelah disetujui)</span></span>
                        </label>
