...
export default function Create() {
    const { data, setData, post, errors, processing } = useForm({ nama: '', tipe: 'pemasukan', deskripsi: '', is_active: true });

    const submit = (e) => { e.preventDefault(); post(route('admin.kategori.store')); };

    return (
        <AuthenticatedLayout header={<div><h1 className="text-xl font-bold text-gray-900">Tambah Kategori</h1><p className="mt-0.5 text-[13px] text-gray-500">Buat kategori keuangan baru</p></div>}>
                ...
                <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                    <form onSubmit={submit} className="space-y-5">
                        <div>
                            ...
                            <TextInput value={data.nama} onChange={(e) => setData('nama', e.target.value)} className="mt-1.5 block w-full" placeholder="Contoh: SPP Bulanan" />
                        ...
                        <div>
                            ...
                            <TextInput value={data.deskripsi} onChange={(e) => setData('deskripsi', e.target.value)} className="mt-1.5 block w-full" placeholder="Deskripsi singkat..." />
